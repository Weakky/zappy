...
** Last update Tue Jun 27 10:45:11 2017 clément
*/
...
int	circular_buffer_push(t_circular_buffer *buffer,
			     void const *ptr, const size_t ptr_size)
{
  if (ptr_size > buffer->elm_size
      || buffer->count >= buffer->capacity)
    return (-1);
  bzero(buffer->head, buffer->elm_size);
  memcpy(buffer->head, ptr, ptr_size);
